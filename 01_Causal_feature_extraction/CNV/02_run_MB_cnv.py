...
# PATHS FOR CNV
FILTERED_DIR = r"C:\Users\olegk\Desktop\Thesis_v3\01_Causal_feature_extraction\CNV\statistical_filtered"
OUTPUT_DIR = r"C:\Users\olegk\Desktop\Thesis_v3\01_Causal_feature_extraction\CNV\mb_results"

os.makedirs(OUTPUT_DIR, exist_ok=True)

MB_MIN_FEATURES = 50  # minimum genes in MB output; pad with composite fallback if fewer
...
def process_single_config(dataset_name, rna_df, outcome, algorithm, alpha, pbar, original_features):
    ...
    causal_genes = [feature_names[i] for i in indices]
    # Fallback: if MB selected fewer than MB_MIN_FEATURES, pad with composite-ranked features
    if len(causal_genes) < MB_MIN_FEATURES:
        _cache = os.path.join(FILTERED_DIR, '..', 'cnv_statistics_cache.csv')
        if os.path.exists(_cache):
            _stats = pd.read_csv(_cache)
            _existing = set(causal_genes)
            _pool = [g for g in _stats['gene'].tolist()
                     if g in feature_names and g not in _existing]
            _needed = MB_MIN_FEATURES - len(causal_genes)
            causal_genes = causal_genes + _pool[:_needed]
            print(f"   Padded to {len(causal_genes)} features (composite fallback)")



    # Fallback: if MB selected fewer than MB_MIN_FEATURES, pad with top composite genes
    if len(causal_genes) < MB_MIN_FEATURES:
        stats_cache = os.path.join(FILTERED_DIR, '..', 'cnv_statistics_cache.csv')
        if os.path.exists(stats_cache):
            stats_df = pd.read_csv(stats_cache)
            existing = set(causal_genes)
            top_genes = [g for g in stats_df['gene'].tolist()
                         if g in feature_names and g not in existing]
            needed = MB_MIN_FEATURES - len(causal_genes)
            causal_genes = causal_genes + top_genes[:needed]
            print(f"   Padded from {len(causal_genes)-needed} to {len(causal_genes)} genes (composite fallback)")

    metrics = evaluate_genes(rna_df, outcome['OS.time'], causal_genes)
    
    print(f"\n   {algorithm:10s} α={alpha}: {len(causal_genes):3d} genes, MI={metrics['mean_mi']:.4f}, C-index={metrics['c_index']:.4f}, {elapsed:.1f}s ({elapsed/60:.1f}min)")
            ...
            for algo in algorithms:
                ...
                if result:
                    ...
                    algo_results_by_alpha[alpha][algo] = {
                        'indices': [rna_df.columns.get_loc(g) for g in result['causal_genes']],
                        'genes': result['causal_genes']
                    }
                    
                    genes_file = os.path.join(dataset_output, f'{algo}_alpha{alpha}_genes.txt')
                    with open(genes_file, 'w') as f:
                        f.write('\n'.join(result['causal_genes']))
...
if all_results:
    ...
    display_df = summary_df.drop(columns=['causal_genes'], errors='ignore')
    ...
    for algo in sorted(summary_df['algorithm'].unique()):
        ...
        print(f"  {algo:10s}: avg {algo_df['n_causal_genes'].mean():.1f} genes, "
    ...
    for alpha in ALPHAS:
        ...
        if len(alpha_df) > 0:
            print(f"  α={alpha}: avg {alpha_df['n_causal_genes'].mean():.1f} genes, "
...
print("\n" + "="*80)
print("CNV PRODUCTION RUN COMPLETE!")
