...
# PATHS FOR MIRNA
FILTERED_DIR = r"C:\Users\olegk\Desktop\Thesis_v3\01_Causal_feature_extraction\miRNA\statistical_filtered"
OUTPUT_DIR = r"C:\Users\olegk\Desktop\Thesis_v3\01_Causal_feature_extraction\miRNA\mb_results"
...
def process_single_config(dataset_name, rna_df, outcome, algorithm, alpha, pbar, original_features):
    ...
    causal_mirnas = [feature_names[i] for i in indices]
    # Fallback: if MB selected fewer than MB_MIN_FEATURES, pad with composite-ranked features
    if len(causal_mirnas) < MB_MIN_FEATURES:
        _cache = os.path.join(FILTERED_DIR, '..', 'mirna_statistics_cache.csv')
        if os.path.exists(_cache):
            _stats = pd.read_csv(_cache)
            _existing = set(causal_mirnas)
            _pool = [g for g in _stats['mirna'].tolist()
                     if g in feature_names and g not in _existing]
            _needed = MB_MIN_FEATURES - len(causal_mirnas)
            causal_mirnas = causal_mirnas + _pool[:_needed]
            print(f"   Padded to {len(causal_mirnas)} features (composite fallback)")


    metrics = evaluate_genes(rna_df, outcome['OS.time'], causal_mirnas)
    
    print(f"\n   {algorithm:10s} α={alpha}: {len(causal_mirnas):3d} genes, MI={metrics['mean_mi']:.4f}, C-index={metrics['c_index']:.4f}, {elapsed:.1f}s ({elapsed/60:.1f}min)")
            ...
            for algo in algorithms:
                ...
                if result:
                    ...
                    algo_results_by_alpha[alpha][algo] = {
                        'indices': [rna_df.columns.get_loc(g) for g in result['causal_mirnas']],
                        'genes': result['causal_mirnas']
                    }
                    
                    genes_file = os.path.join(dataset_output, f'{algo}_alpha{alpha}_genes.txt')
                    with open(genes_file, 'w') as f:
                        f.write('\n'.join(result['causal_mirnas']))
...
print("\n" + "="*80)
print("PROCESSING MIRNA DATASETS")
print("="*80)
...
if all_results:
    ...
    display_df = summary_df.drop(columns=['causal_mirnas'], errors='ignore')
    ...
    for algo in sorted(summary_df['algorithm'].unique()):
        ...
        print(f"  {algo:10s}: avg {algo_df['n_causal_mirnas'].mean():.1f} genes, "
    ...
    for alpha in ALPHAS:
        ...
        if len(alpha_df) > 0:
            print(f"  α={alpha}: avg {alpha_df['n_causal_mirnas'].mean():.1f} genes, "
...
print("\n" + "="*80)
print("MIRNA PRODUCTION RUN COMPLETE!")
