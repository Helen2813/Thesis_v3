...
for short_name in DATASET_NAMES:
    ...
    for algorithm, alpha in product(ALGORITHMS, ALPHAS):
        ...
        # ------------------------------------------------------------------
        # CAUSAL STABILITY — timeout = 1 hour (skip only this if exceeded)
        # ------------------------------------------------------------------
        stability = None
        stab_elapsed = 0.0
        rng = np.random.default_rng(RANDOM_SEED)

        print(f"  Stability bootstrap (n={N_BOOTSTRAP})...", end="", flush=True)
        t_stab = time.time()
        selection_counts = {f: 0 for f in selected}
        n_completed = 0

        stab_timed_out = False
        for b in range(N_BOOTSTRAP):
            if time.time() - t_stab > STAB_TIME_LIMIT_SEC:
                stab_timed_out = True
                break
            idx = rng.choice(len(df), size=int(len(df) * BOOTSTRAP_FRAC), replace=True)
            sub = df.iloc[idx]
            try:
                boot_feats = run_mb(sub[feat_cols], sub["OS.time"].values,
                                    algorithm, alpha)
                for f in boot_feats:
        ...
        stab_elapsed = time.time() - t_stab

        if stab_timed_out:
            msg = (f"STABILITY TIMEOUT: {short_name} | {combo_str} | "
                   f"completed {n_completed}/{N_BOOTSTRAP} iterations "
                   f"({stab_elapsed/3600:.2f}h) — stability skipped, "
                   f"C-index={c_idx:.4f}, AUC-5yr={auc5:.4f} retained")
            print(f" TIMEOUT ({n_completed}/{N_BOOTSTRAP} done)")
            skip_stb_log.append(msg)
        else:
            scores = [selection_counts[f] / N_BOOTSTRAP for f in selected]
            stability = float(np.mean(scores)) if scores else 0.0
            print(f" {stability:.4f}  ({stab_elapsed:.1f}s)")
