...
Input: MERGE_continuous_outer/ (continuous RNA + outer join + missingness indicators)
"""
...
OUT_DIR   = SCRIPT_DIR / "results_continuous_outer"
...
if not all_results:
    ...
    results_df.to_csv(OUT_DIR / "mb_results_continuous_outer.csv", index=False)
    print(f"\nSaved: mb_results_continuous_outer.csv")
    ...
    import json
    with open(OUT_DIR / "best_config_continuous_outer.json", "w") as f:
        json.dump(best_detail, f, indent=2)
    print(f"\nSaved: best_config_continuous_outer.json")
    ...
    if skip_mb_log or skip_stb_log:
        ...
        with open(OUT_DIR / "timeout_log_continuous_outer.txt", "w") as f:
            f.write("MB TIMEOUTS\n" + "\n".join(skip_mb_log))
            f.write("\n\nSTABILITY TIMEOUTS\n" + "\n".join(skip_stb_log))
        print(f"\nSaved: timeout_log_continuous_outer.txt")
