...
def load_stats_cache():
    ...
    print("  WARNING: stats cache not found - padding disabled")
    return []


def pad_genes(selected, feature_names, min_n, ranked_pool):
    if len(selected) >= min_n:
        return selected
    have   = set(selected)
    extras = [g for g in ranked_pool if g in set(feature_names) and g not in have]
    padded = selected + extras[:min_n - len(selected)]
    print(f"  Padded: {len(selected)} -> {len(padded)} genes (min={min_n})")
    return padded
...
ranked_pool = load_stats_cache()
print(f"  Outcome : {len(outcome)} samples, "
      f"{int(outcome['OS'].sum())} events ({outcome['OS'].mean()*100:.1f}%)")
print(f"  Cache   : {len(ranked_pool)} genes available for padding")
