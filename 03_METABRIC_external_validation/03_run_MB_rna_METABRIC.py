# -*- coding: utf-8 -*-
"""
MARKOV BLANKET - Mutations (METABRIC)
======================================
...
FILTERED_DIR      = str(_MUT / "statistical_filtered")
STATS_CACHE       = str(_MUT / "mut_statistics_cache.csv")
DATASET_PATTERN   = "mut_8_composite"
ALGORITHM         = "IAMB"
ALPHA             = 0.05
MB_MIN_THRESHOLDS = [100, 200]

OUTPUT_DIRS = {
    100: str(_MUT / "mb_results_min100"),
    200: str(_MUT / "mb_results_min200"),
}
...
print("=" * 70)
print("MARKOV BLANKET - Mutations (METABRIC)")
print("=" * 70)
...
def load_fdr_pool():
    ...
    # Falls back to full composite ranking if stats file not found
    stats_complete = os.path.join(FILTERED_DIR, "mut_statistics_complete.csv")
    if os.path.exists(stats_complete):
        ...
        if fdr_genes:
            ...
            return fdr_genes
        # If no FDR<0.05 genes, fall back to full composite ranking
        print("  WARNING: no FDR<0.05 genes found - using full composite ranking for padding")
...
def evaluate_genes(mut_df, outcome, gene_names):
    if not gene_names:
...
def run_iamb(mut_df, outcome, mb_min, ranked_pool, output_dir):
    feature_names = mut_df.columns.tolist()

    # Add tiny noise for numerical stability (IAMB uses continuous CI tests)
    X = mut_df.values.astype(float) + np.random.normal(0.0, 1e-8, mut_df.shape)
    ...
    causal_genes = pad_genes(raw_genes, feature_names, mb_min, ranked_pool)
    metrics      = evaluate_genes(mut_df, outcome, causal_genes)
...
ranked_pool = load_fdr_pool()
print(f"  Outcome : {len(outcome)} samples, "
      f"{int(outcome['OS'].sum())} events ({outcome['OS'].mean()*100:.1f}%)")
print(f"  FDR pool: {len(ranked_pool)} genes available for padding")
...
mut_df = pd.read_csv(os.path.join(FILTERED_DIR, dataset_file), index_col=0)
print(f"Shape   : {mut_df.shape[0]} samples x {mut_df.shape[1]:,} genes")

common  = sorted(set(mut_df.index) & set(outcome.index))
mut_df  = mut_df.loc[common]
...
for mb_min in MB_MIN_THRESHOLDS:
    ...
    r = run_iamb(mut_df, outcome, mb_min, ranked_pool, out_dir)
    if r:
...
if all_results:
    out_csv = str(_MUT / "mb_comparison_min100_vs_min200.csv")
